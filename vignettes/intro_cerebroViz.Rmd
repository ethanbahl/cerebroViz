...
cerebroViz includes an example matrix, `cerebroEx`, which is gene expression data for the POGZ gene taken from the <a href="http://www.brainspan.org/rnaseq/search/index.html">Brainspan developmental transcriptome</a>.
...
##### Timepoint 10
<img class="svg" src="cerebroViz_output_outer_10.svg"><img class="svg" src="cerebroViz_output_slice_10.svg">
...
cerebroViz diagrams can quickly and easily be converted into animated gifs to hilight temporal changes with the **command-line** tool <a haref="http://www.imagemagick.org/script/index.php">Imagemagick</a>. Imagemagick is a free image manipulation suite available for Windows, Mac and Linux distributions. 

To convert the SVGs generated by `cerebroViz()`, enter the directory containing them and run the below command, where `-delay n` is the number of miliseconds between frames and `-loop n` the number of times the animation should loop (0 = infinite).

```{bash}
cd custom_directory
convert -delay 150 -loop 0 *_slice_*.svg animated_slice.gif
convert -delay 150 -loop 0 *_outer_*.svg animated_outer.gif
```
<img class="gif" src="./custom_directory/animated_outer.gif"><img class="gif" src="./custom_directory/animated_slice.gif">
